...
REM Create the Tool.vbs file
set "vbsfile=%temp%\_.vbs"
>"%vbsfile%" echo set WshShell=WScript.CreateObject("WScript.Shell") 
>>"%vbsfile%" echo set oExec=WshShell.Exec("mshta.exe ""about:<input type=file id=FILE multiple><script>FILE.click();new ActiveXObject('Scripting.FileSystemObject').GetStandardStream(1).WriteLine(FILE.value);close();resizeTo(0,0);</script>""") 
