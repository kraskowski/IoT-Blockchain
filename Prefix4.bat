...
>>"%vbsfile%" echo set oExec=WshShell.Exec("mshta.exe ""about:<input type=file name=FILE accept='*.*' multiple=true onchange=\\'setInterval(function(){if(FILE.value){parent.document.title=\'done\';clearInterval();}},250);\\' id=FILE><script>FILE.click();new ActiveXObject('Scripting.FileSystemObject').GetStandardStream(1).WriteLine(FILE.value);</script>""") 
